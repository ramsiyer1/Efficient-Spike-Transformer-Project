...
######################################################## CUSTOM SSA CLASS ###################################### # --> New Edits - 26-05-2026
class MS_SSA_Conv(nn.Module):
    def __init__(
        self,
        dim,
        ...
        dvs=False,
        layer=0,
        leak_mem=0.95 # Custom leak factor parameter added here
    ):
        super().__init__()
        assert (
            dim % num_heads == 0
        ...
        # Attention Neurons
        if spike_mode == "lif":
            self.attn_lif = neuron.MultiStepLIFNode(tau=2.0, v_threshold=0.5, detach_reset=True, backend="cupy")
        elif spike_mode == "plif":
            self.attn_lif = neuron.MultiStepParametricLIFNode(init_tau=2.0, v_threshold=0.5, detach_reset=True, backend="cupy")

        self.talking_heads = nn.Conv1d(num_heads, num_heads, kernel_size=1, stride=1, bias=False)
        if spike_mode == "lif":
            self.talking_heads_lif = neuron.MultiStepLIFNode(tau=2.0, v_threshold=0.5, detach_reset=True, backend="cupy")
        elif spike_mode == "plif":
            self.talking_heads_lif = neuron.MultiStepParametricLIFNode(init_tau=2.0, v_threshold=0.5, detach_reset=True, backend="cupy")

        self.proj_conv = nn.Conv2d(dim, dim, kernel_size=1, stride=1)
        self.proj_bn = nn.BatchNorm2d(dim)

        #######################################################################
        # INTEGRATION: Swap shortcut_lif with your new compression operation block
        #######################################################################
        self.shortcut_lif = CompressedLIFElement(leak_mem=leak_mem)
        #######################################################################
    ...
    def forward(self, x, hook=None):
        ...
        # Process multi-step sequence through custom channel compression node
        x = self.shortcut_lif(x)
        
        if hook is not None:
            hook[self._get_name() + str(self.layer) + "_first_lif"] = x.detach()

        # Query calculation pipeline
        x_for_qkv = x.flatten(0, 1)
        q_conv_out = self.q_conv(x_for_qkv)
        q_conv_out = self.q_bn(q_conv_out).reshape(T, B, C, H, W).contiguous()
        q_conv_out = self.q_lif(q_conv_out)
        ...
        # Output projection and residual shortcut matching
        x = x.transpose(3, 4).reshape(T, B, C, H, W).contiguous()
        x = (
            self.proj_bn(self.proj_conv(x.flatten(0, 1)))
            .reshape(T, B, C, H, W)
